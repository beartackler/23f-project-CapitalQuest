...
@internships_bp.route('/internships_available/<int:internship_id>', methods=['GET'])
...
@internships_bp.route('/internships_available/company/<string:company_name>', methods=['DELETE'])
def delete_internships_for_company(company_name):
    # delete all internships for a specific company
    cursor = db.get_db().cursor()
    cursor.execute('DELETE FROM internships WHERE company_name = ?', (company_name,))
    db.get_db().commit()
    return jsonify({'message': 'Internships deleted for company {}'.format(company_name)})

@internships_bp.route('/internships_available/<int:internship_id>', methods=['PUT'])
def update_specific_internship(internship_id):
    # update a specific internship
    internship_data = request.json
    cursor = db.get_db().cursor()
    cursor.execute('UPDATE internships SET title = ?, description = ?, other_details = ? WHERE id = ?',
